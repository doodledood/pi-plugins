...
**Unreported spend**:
Paid work whose tool result reports no usage, so nothing in the session records it and no total can include it — web-search answer synthesis and image generation are the usual cases.
_Avoid_: Untracked cost when the cause (no reported usage) matters; naming specific packages, since which are installed is not knowable from the cost surfaces.
