...
- `settings.example.json` — normal installed setup. It tracks `git:github.com/doodledood/pi-plugins@main`, installs external helper packages, defaults to `anthropic/claude-opus-5` at high thinking, and limits the model cycle to Sol at high, Opus 5 at high, Luna at max, and Fable at high, plus unrestricted `claude-opus-5-full` and `claude-fable-5-full` variants at high for deliberate full-window sessions. Sol uses a 240K visible / 1.05M target dual-window alias, Luna a 272K one, Opus 5 and Fable 5 use 350K visible / 1M target ones, and the `-full` variants expose the whole 1M window, the setup enables `deep-focus-pi`, and image generation uses `gpt-image-2`.
...
- `models.example.json` — empty model-provider template. The full profile keeps every dual-window alias — Sol, Luna, Opus 5, and Fable 5 — in `configs/model-aliases.json` instead of using model-provider overrides.
...
4. Run `npm run sync:doctrine -- --install` to place HQ doctrine on a machine that has none;
   ...
   reviewing them. The Sol entry in `configs/model-aliases.json` enforces a 240K operating boundary (Luna 272K) through Pi's automatic compaction and native compact-and-retry paths while delegating allowed requests with a 1.05M target window; the Opus 5 and Fable 5 entries do the same at a 350K boundary against their 1M target window, and the `claude-opus-5-full` / `claude-fable-5-full` aliases leave the full 1M window unrestricted for sessions that deliberately want it. The Anthropic aliases carry no `apiKey`, so they use whatever auth that provider already has, falling back to `$ANTHROPIC_API_KEY`. The goal-controller config deliberately overrides only checker model and thinking, leaving all other controller settings on package defaults.
