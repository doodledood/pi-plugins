...
It resolves the clone's upstream commit, writes it to `/tmp/manifest-dev/.source-commit`, and stages that commit's tree — not the working tree, which goes stale the moment a fetch lands without a checkout. Pass the recorded sha to `sync.py` rather than reading the clone's `HEAD`, which names the sync's own commit whenever a session has that clone parked on a branch of its own. A stale checkout is therefore safe: fetch is enough, and no `git pull` is needed. The recorded value becomes `source_commit` in every tracking file, and diffing it against the previous one gives you the upstream commit list for the PR body. On either CDN path there is no sha to record, so the file says `unresolved-…` instead: the sync still works, but that entry is one no later run can diff against.
