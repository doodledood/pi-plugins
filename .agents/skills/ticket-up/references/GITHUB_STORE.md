...
| Escalated attempt | Add the detailed handoff comment, leave the issue open with `auto` unchanged, and assign the person needed next. After resolving the blocker, the person records continuation context and unassigns the issue. If no person can be resolved, preserve an assignee and state the human handoff explicitly so the issue does not return to the ready set |
