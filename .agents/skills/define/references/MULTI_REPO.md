...
*What it is, and how it is exercised end-to-end:* the SSO entry point, exercised by posting a
real SAML assertion to a running backend and following the session it hands back.

#### AC-1.1 — POST /auth/sso accepts a SAML assertion

Done when posting a test SAML assertion to the backend repo's running service
(`curl -X POST http://localhost:8080/auth/sso -d @/tmp/saml-test.xml`) returns 200 with a
session cookie set.
...
The map is wrapper context, never part of a gate's own text — it travels with the run like the eligible-gate set and the framing line, and changes no gate's threshold. Single-repo manifests get no map.

## /done and /auto for multi-repo

/done fires **once per manifest**, including for multi-repo. No per-repo /done independence. Every AC + every Global Invariant across every repo's deliverables must have fresh PASS evidence under the selected mode (with no BLOCKED pending) before /done is called. The cross-repo wrapper above is what makes a single /done reachable: every evaluator has all repo paths, so cross-repo behavior is checkable during normal /do flow. Multi-repo /done summary lists which repos' deliverables were verified.

/auto's chain (figure-out → define → do) covers the whole multi-repo implementation phase in one invocation, navigating between repos as deliverables require. Lifecycle tending is part of /do's execution when the manifest carries lifecycle ACs whose bodies activate the `check-pr` skill under the selected mode (PR_LIFECYCLE.md auto-templates one per repo). `/auto --babysit <pr-url>` is single-PR by construction; for multi-repo lifecycle tending, declare `Repos:` in the manifest and let /define template the per-repo ACs.
