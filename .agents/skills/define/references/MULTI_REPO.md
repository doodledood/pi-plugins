...
When the manifest declares `Repos:`, `/do` adds the cross-repo path map as wrapper context around each verbatim `verify.instructions` block so every verification mode has access to all repos' paths:

```
Available repos: backend=/home/user/projects/api, frontend=/home/user/projects/web

[criterion's verify.instructions: follows verbatim]
```
