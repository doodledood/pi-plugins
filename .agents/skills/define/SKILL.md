...
Say how each slice is exercised by naming the situation it is put in front of — a request served end to end, a reader reaching the passage they came for. *It compiles*, *the tests pass*, *the file exists*, *the section is written* are inspections of the artifact rather than uses of it. An exercise line reading that way means one of two things, and they take different repairs: the slice has a use you haven't named, so name it; or it has none to be put in front of, so re-cut it. Where the work arrives already framed as layers — a request that asks for the schema first, then the endpoints, then the screen — that framing is the requester's convenience rather than the cut: re-cut it into slices that each run, instead of encoding the layers as handed. Renaming a layer does not re-cut it.
...
| UI | User-visible artifacts of any genre: screens, pages, components, styling, dashboards, rendered documents, decks, infographics, posters, forms, emails, games, explorables — anything a person sees and judges rather than calls | `UI.md` |
...
| Writing | Human-voice register: prose, articles, copy, social, creative (base: Prose floor) | `WRITING.md` |
| Document | Documentation register: specs, proposals, reports, formal docs (base: Prose floor) | `DOCUMENT.md` |
...
| Marketing | Landing pages, launch posts, ads, sales/pricing copy, positioning pages, go-to-market assets (base: Writing) | `MARKETING.md` |

*Composition:* code-change tasks combine `CODING.md` (base gates) with the specific FEATURE/BUG/REFACTOR, adding `UI.md` when the change touches a user-visible artifact; `UI.md` also applies on its own when the deliverable is a non-code visual artifact (a deck, an infographic, a rendered document); text-authoring combines `PROSE_FLOOR.md` with exactly one register overlay — `WRITING.md` for human-voiced prose (BLOG and MARKETING compose onto it) or `DOCUMENT.md` for documentation (TECH_DESIGN composes onto it). **The two registers never compose together:** their rules contradict each other by design — uniform terminology against vocabulary diversity, consistency against deliberate imperfection — so a deliverable takes the register of its dominant body and the other's gates do not apply. Research composes `research/RESEARCH.md` with `research/sources/`. Domains aren't mutually exclusive (a bug fix that refactors uses both). `PR_LIFECYCLE.md` composes when the output ships through a GitHub PR (auto-detected from a github.com `origin`; probe if origin is missing or a github-enterprise host), including tech-design docs shipped as PRs — it templates one AC per repo whose body activates the `check-pr` skill under whichever `/do` verification mode is selected. That body is the steering surface for per-PR nuances (labels, approvers, flaky-CI/retrigger overrides). **Exception:** PROMPTING does not compose with CODING unless the task also changes executable code.
