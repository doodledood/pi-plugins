...
- `../design/references/figures.md` — what a figure depicts, how options are compared, the inline-SVG mechanics, charts to one scale.
...
5. **Judge the renders** against the loaded standards, in this order: task fit — whether the arrangement lets that loop run, with what the loop needs together visible together, the repeated action reachable without hunting, and the sequence's order matching the reading order, and whether each structural or comparative claim is carried by the encoding the loop needs: a figure where a cold reader would otherwise assemble the mechanism from prose (where data flows, which parts talk, what changes between two options), a table where several things are compared on the same attributes, prose where a sentence is faster — with a figure that merely restates a labeled list, or a paragraph left beside the figure that encodes it, judged as encoding failures too; register fit; functional floors (states, error paths, recovery); composition and hierarchy, including the blur test as the standards specify it; craft consistency (spacing rhythm, alignment spine, palette discipline, type); copy; where the artifact claims distinctiveness, whether its signature element derives from the subject or from the overused-looks list.
