...
- `per-gate` — a fresh independent verifier execution per gate. Highest quality, slowest, dearest: each gate gets a whole context to itself.
- `consolidated` *(default)* — one independent verifier execution for all outstanding gates. Middle on all three: it splits its attention across the set.
