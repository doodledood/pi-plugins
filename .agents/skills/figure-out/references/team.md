...
**Entry.** Required: channel-or-thread and the named participants. Infer from context (the project context file, conversation, repo config); ask the operator once in the local chat session (never in Slack) if you can't. Same inference-first-then-ask for topic, owner handle, and roles. Prerequisite: Slack read/post tooling wired into the operator's session — fail fast with clear remediation if absent.
