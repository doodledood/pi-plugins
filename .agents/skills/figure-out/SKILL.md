...
**Advance one claim per turn**; the rest of what you found waits for its own turn. Bold lines are the parts of that one claim, not separate findings — three or four at most, and reading them alone should lose nothing: they are the three-second version of the turn, written for sparse attention. Keep the prose under each bold to a sentence or two. What makes a turn heavy is the second and third proof of a point it already carried: cut those, never the claim. The read-naming turn is the exception to the count alone — its full anatomy travels together, because no turn follows it, but each part still gets its sentence or two.
...
Some sessions reach a point where the remaining unresolved questions stop depending on each other — none needs another's answer, and no single read will cohere them. Pressing on serially buys nothing there: name what you're seeing and offer to scope the read to the settled core, handing the decoupled questions off as question tickets in the project's ticket store. On accept, hold the tickets back until the read is named — a still-moving session can reshape a parked question, and a stale ticket in a shared store may already be claimed — then emit them together with the read, each carrying the question, why it matters, and what's already known, loading the ticket-up skill's ticket convention at that point. When the work the read sets in motion will return verdicts that need judging, emit the successor question with them — what the verdicts change and what moves next — depending on the tickets it will judge, so the next deliberation is entered from the store rather than from someone remembering to think. Wire its edges to fellow question tickets directly; where it judges shaped work the manifest will produce, name the successor in the read and leave the wiring to ticket-up, which mints those tickets' IDs. Only question tickets leave a figure-out session; shaped tickets come from ticket-up over a finished manifest. An offer, not a switch; the trigger is the observed decoupling, never the topic's size, and declining it means keep pressing as usual.
