...
- `models.example.json`: empty model-provider template; GPT-5.5 1M is configured under the settings examples' `model-aliases` section instead of a global built-in model override.
...
`settings.upstream.example.json` pins this repo as `git:github.com/doodledood/pi-plugins@v0.5.0`. That is the preferred live setup: custom extensions come from the upstream Git package/tag, while local bare extension files remain reserved for external installers such as Herdr.
