...
- `mcp.example.json`: MCP setup template with placeholders for local wrapper paths, proxy IDs, and API keys.
...
5. Fill MCP placeholders locally; never commit filled-in tokens, OAuth state, or raw `auth.json`.
