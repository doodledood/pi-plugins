...
- `auth.example.json`: secret-free template for `~/.pi/agent/auth.json` that scopes `PI_CACHE_RETENTION=long` to the OpenAI provider only (24h prompt-cache retention; the `$OPENAI_API_KEY` value is env-indirection, no secret stored). Anthropic intentionally stays on the default 5-minute cache TTL — its 1h tier doubles every cache write, and the `cache-optimization` extension's TTL keepalive covers in-flight gaps far cheaper. Do not set `PI_CACHE_RETENTION` in your shell environment; that would flip Anthropic to 1h too.
