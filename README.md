...
6. **Verify the effective setup**
   ...
   - Run `pi --list-models`; for the full profile confirm `anthropic/claude-fable-5`, `openai/gpt-5.6-sol`, and `openai/gpt-5.6-luna` are available. Confirm the configured model cycle contains only Fable at medium, Sol at xhigh, and Luna at max; Sol reports 372K context, Luna reports approximately 1.1M, and `model-aliases.json` gives both OpenAI models a 1,050,000-token target window.
