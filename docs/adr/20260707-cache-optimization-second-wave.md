...
Gap-end decomposition showed that true foreground in-flight waits were small (about 3 events / ~$6). The dominant Anthropic TTL shape was background wakeups and idle gaps: background completion entries and custom entries arrived after more than 5 minutes with no foreground tool execution still in flight. The actionable thinking-off background-wakeup slice was estimated at about $25–28 per 14 days. At the time, thinking-enabled TTL gaps were treated as structurally unreachable because budget-style extended thinking cannot be refreshed by a 1-token ping. This was later narrowed by `20260707-adaptive-thinking-ttl-keepalive.md`: modern adaptive-thinking payloads can be refreshed by exact captured-provider-payload replay with streaming usage proof, while budget-style `thinking.enabled` remains excluded.
...
1. **Generalize TTL keepalive to background work without package coupling.**
   ...
   - Existing keepalive bounds remain: same-route Anthropic API-key route, 5m-TTL payloads only, thinking-safe payloads only (no-thinking/disabled plus, as later added by `20260707-adaptive-thinking-ttl-keepalive.md`, adaptive-thinking captured-payload replay; budget-style thinking excluded), 100k prompt floor, per-gap ping cap, daily dollar cap, and zero pings while idle with no foreground/background work.
