...
Pi JSON-mode process success does not count as a checker verdict by itself. The controller separately classifies terminal assistant/provider errors, empty assistant responses, malformed event streams, and invalid verdict text. The event-stream check covers only the verdict path — every line is typed JSON, the run settles once with nothing after it, and each assistant `message_end` is finished with readable content — and ignores event types, message roles, content blocks, and fields Pi adds, so a Pi release that extends its JSON output does not pause goals. A malformed stream is reported by violation kind (for example `1× event after agent_settled`). A checker execution failure pauses the goal with the inherited model or explicitly requested model pattern and a fixed, secret-safe failure classification; raw provider, process, verdict, stderr, and JSONL text is not persisted or shown; when an inherited model cannot perform checker work, set `checker.model` explicitly in the config file. Missing configuration continues to default to `inherit`.
