...
export const GOAL_DESCRIPTION =
  ...
  "goal always creates a fresh goal. It can be called when no live goal is active, including after a paused, blocked, budget-limited, or completed goal; the new goal becomes current while the prior non-live goal remains history. " +
  "goal never updates, edits, clears, pauses, resumes, or completes any existing goal.";

export const GOAL_GUIDELINES = [
  "Use goal only when no live goal is active and the work may require multiple turns toward a verifiable end state. Completed, paused, blocked, and budget-limited goals are not live, so a fresh goal may be started after those statuses; starting fresh does not update, resume, or complete the prior goal.",
