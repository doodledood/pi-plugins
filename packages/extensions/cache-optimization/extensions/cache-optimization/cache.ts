// cache.ts — pure cache-efficiency analysis for the cache-optimization extension.
// (Originally extracted from simple-statusline; the "visible" gating fields are
// still exported for the statusline-compatible stats consumers and tests.)
...
/** Hide the ambient metric until the branch has at least this many prompt tokens (statusline-compatible gate). */
...
export interface SessionCacheStats {
  ...
  sessionRate: number;
  /** Ambient-metric visibility gate (consumed by the statusline package's copy). */
  visible: boolean;
...
/** Aggregate branch cache stats (session rate, visibility gate, latest-break flag). */
...
/** Compact token count for report display. */
...
/** Whole-number percentage for report display. */
export function pct(rate: number): string {
  return `${Math.round(rate * 100)}%`;
}

/** Dollar cost for report display. */
