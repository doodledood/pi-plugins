...
- While open, BTW temporarily enables standard SGR mouse reporting through a process-global cooperative lease shared with compatible Pi overlays, so one overlay cannot disable reporting while another remains open. Terminals without SGR mouse support retain keyboard controls but not click focus or wheel scrolling. Clicks switch focus but do not reposition the editor cursor.
